<template>
  <div class="map-page">
    <div class="toolbar">
      ...
      <span class="meta">视野内 {{ points.length }} 条案件</span>
    </div>
    <div class="map-wrap">
...
import { fetchMapConfig, queryCaseMapPoints } from '../api/http'
...
function popupHtml(item) {
  return [
    `<strong>${escapeHtml(item.ajmc || '-')}</strong>`,
    item.asjbh ? `<br/>案事件编号：${escapeHtml(item.asjbh)}` : '',
    item.ajfsdxzqhmc ? `<br/>行政区划：${escapeHtml(item.ajfsdxzqhmc)}` : '',
    item.ajfsdzmc ? `<br/>发案地址：${escapeHtml(item.ajfsdzmc)}` : '',
    item.slsj ? `<br/>受理时间：${escapeHtml(fmtTime(item.slsj))}` : '',
    item.jyaq ? `<br/>简要案情：${escapeHtml(item.jyaq.length > 80 ? item.jyaq.slice(0, 80) + '…' : item.jyaq)}` : ''
  ].join('')
}
...
async function reload() {
  if (!map) return
  const b = queryBounds()
  const resp = await queryCaseMapPoints({
    minLng: b.minLng,
    minLat: b.minLat,
    maxLng: b.maxLng,
    maxLat: b.maxLat,
